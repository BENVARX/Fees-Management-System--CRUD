...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $sql_update_feesntries= "UPDATE feesntries
    ...
    if ($conn->query($sql_update_feesntries) === TRUE) {
        ...
        $sql_insert_paidfees = "INSERT INTO paidfees (rollno, date,transactionid, admission_fee, tuition_fee, book_fee, uniform_fee, hostel_fee, caution_deposit_hostel, transport_fee, examination_fee, library_fee, placement_training_fee, others_fee, total)
                                VALUES ('$rollno', '$date','$transactionid', '$admission_fee', '$tuition_fee', '$book_fee', '$uniform_fee', '$hostel_fee', '$caution_deposit_hostel', '$transport_fee', '$examination_fee', '$library_fee', '$placement_training_fee', '$others_fee', '$total')";

        if ($conn->query($sql_insert_paidfees) === TRUE) {
            echo "Values inserted into paidfees table successfully";
            header("location:recipttdisp.php");
        } else {
